...
namespace SGA.Persistence
{
    public static class DbInitializer
    {
        public static void Seed(AppDbContext context)
        {
            ...
            {
                Console.WriteLine("⚠️ Los datos ya existen, saltando seed...");
                return;
            }
            ...
            for (int i = 1; i <= 500; i++)
            {
                ...
                libros.Add(new Libro
                {
                    ...
                    ISBN = $"ISBN-{random.Next(100000, 999999)}",
                    Ubicacion = $"Estante {random.Next(1, 10)}",
                    Editorial = $"Editorial {random.Next(1, 20)}",
                    Stock = random.Next(1, 20),
                    StockDisponible = random.Next(1, 15),
                    Estado = Domain.Enums.EstadoRecurso.Disponible,
                    FechaAdquisicion = DateTime.Now.AddDays(-random.Next(1, 365)),
